...
def main():
    ...
    #even_better_draw_circles(rg.Point(100, 50), 20, 5,'aqua',5)
    #even_better_draw_circles(rg.Point(-200, 0), 3, 20,'pink',1)

    even_better_draw_circles(rg.Point(-175, 170), 5, 20, 'black', 1)
    even_better_draw_circles(rg.Point(175, 170), 5, 20,'black',1)
    even_better_draw_circles(rg.Point(0, -50), 10, 20, 'light grey', 3)
...
def better_draw_circles(point,delta):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(delta * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point,delta,num,color,pen_thickness):
    ...
    for k in range(1, num+1):  # k becomes 1, 2, 3, ... num

        turtle.pen_up()

        # Go DOWN num pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(delta)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(delta * k)  # Radius
